...
class DataVisualization extends HTMLElement {
    ...
    show(dataSource, points = [], tableData = null) {
        console.log('显示数据可视化:', dataSource, points, tableData);
        ...
        // 如果有传入的表格数据，直接使用，否则生成模拟数据
        if (tableData) {
            console.log('使用传入的表格数据:', tableData);
            this.processTableData(tableData);
        } else {
            console.log('没有传入表格数据，使用模拟数据');
            this.allData = [];
            this.displayData = [];
            this.currentPage = 1;
    ...
    updateTableHeader(header) {
        const tableHead = this.shadowRoot.querySelector('#dataTable thead tr');
        if (tableHead) {
            // 清空现有表头
            tableHead.innerHTML = '<th>时间</th>';
            
            // 添加动态列头
            header.forEach(columnName => {
                if (columnName !== 'Time') {
                    tableHead.innerHTML += `<th>${columnName}</th>`;
                }
            });
        }
    }
    ...
    async loadData() {
        try {
            ...
            console.log('查询参数:', { startTime, endTime, aggregateType });
    ...
    updateChart() {
        ...
        // 检查是否有Time列和是否为数值数据
        const hasTimeColumn = this.displayData.length > 0 && 
            (this.displayData[0].timestamp || this.displayData[0].Time);
        
        const hasNumericData = this.displayData.some(record => {
            const selectedPointsArray = Array.from(this.selectedPoints);
            return selectedPointsArray.some(point => {
                const value = record[point] !== undefined ? record[point] : record.values && record.values[point] !== undefined ? record.values[point] : null;
                return typeof value === 'number';
            });
        });

        // 如果没有Time列或不是数值数据，不显示曲线图
        if (!hasTimeColumn || !hasNumericData) {
            console.log('没有Time列或非数值数据，隐藏图表');
            const chartContainer = this.shadowRoot.getElementById('chartContainer');
            if (chartContainer) {
                chartContainer.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; color: #999;">
                        ...
                        <div style="font-size: 12px;">${!hasTimeColumn ? '缺少时间列' : '非数值数据'}</div>
                    </div>
                `;
            }
            return;
        ...
        selectedPointsArray.forEach((point, index) => {
            const data = this.displayData.map(record => [
                record.timestamp,
                record[point] !== undefined ? record[point] : record.values && record.values[point] !== undefined ? record.values[point] : 0
            ]);
    ...
    updateTable() {
        ...
        console.log('分页信息：当前页', this.currentPage, '总页数', this.totalPages, '总数据', this.displayData.length);
        ...
        pageData.forEach((record, index) => {
            ...
            selectedPointsArray.forEach(point => {
                const td = document.createElement('td');
                const value = record[point] !== undefined ? record[point] : record.values && record.values[point] !== undefined ? record.values[point] : '-';
    ...
    showDataCleanModal() {
        this.showModal('数据清理', `
            <div class="form-group">
                <label>清理规则：</label>
                <select id="cleanRule">
                    <option value="remove-duplicates">移除重复数据</option>
                    <option value="remove-outliers">移除异常值</option>
                    <option value="fill-missing">填充缺失值</option>
                </select>
            </div>
            <div class="form-group">
                <label>阈值设置：</label>
                <input type="number" id="threshold" placeholder="输入阈值" />
            </div>
        `, () => {
            const rule = this.shadowRoot.getElementById('cleanRule').value;
            const threshold = this.shadowRoot.getElementById('threshold').value;
            console.log('执行数据清理:', rule, threshold);
            // 这里实现实际的数据清理逻辑
        });
    }
